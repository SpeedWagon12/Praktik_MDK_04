...
public class OrdersManager {
    ...
    String getMaxOrderCustomerName() {
        ...
        for (var entry : customersOrders.entrySet()) {
            double sum = 0;
    ...
    void removeUnprofitableOrders() {
        ...
        for (var entry : customersOrders.keySet()) {
            double ordersSum = 0;
            for (Double sumOrder : customersOrders.get(entry)){
                ordersSum += sumOrder;
            }
            if (ordersSum < 5000) {
                names.add(entry);
        ...
        for (var removes : names){
            customersOrders.remove(removes);
            System.out.println("Клиента " + removes + " больше нет в таблице.");
        }
